...
static void keyboard_store_med_name(void)
{
    ...
    /* Per-keyboard-submit Telegram messages removed 2026-05-15 (user:
     * "กดบันทึกอย่างเดียวก็พอ"). Name change still updates the local
     * shadow + persists via the shadow-publish path; the summary message
     * fires once on the meds-detail Save button. */
    if (publish_name[0] == '\0') {
