// ─────────────────────────────────────────────────────────────────────────────
//  netpie_mqtt.c — NETPIE 2020 MQTT client
//  เชื่อมต่อ mqtt.netpie.io, อ่าน/เขียน Shadow data
// ─────────────────────────────────────────────────────────────────────────────
...
uint32_t netpie_mqtt_get_last_rx_time(void) { return s_last_rx_ticks; }
...
static bool shadow_cache_load_locked(void)
{
    ...
    bool ok = (nvs_get_blob(h, SHADOW_CACHE_KEY, &cached, &blob_len) == ESP_OK && blob_len == sizeof(cached));
    nvs_close(h);
    if (!ok) return false;
...
static char *build_shadow_payload_int(const char *key, int value)
{
    ...
    if (!root || !data || !cJSON_AddItemToObject(root, "data", data) ||
...
static void publish_shadow_payload(char *payload)
{
    ...
    if (s_client && s_connected) {
        esp_mqtt_client_publish(s_client, NETPIE_TOPIC_SET, payload, strlen(payload), 0, 0);
        ESP_LOGI(TAG, "Shadow updated: %s", payload);
    } else {
        ESP_LOGI(TAG, "Shadow cached locally (publish deferred): %s", payload);
...
/* ─────────────────────────────────────────────────────────────
   JSON helpers — minimal parser (ไม่ใช้ library ภายนอก)
   อ่านค่า string/int จาก JSON flat object
───────────────────────────────────────────────────────────── */

/** ค้นหา "key":"value" หรือ "key":number แล้วคืน value string
 *  buf_out = output buffer, buf_len = size
 *  คืน true ถ้าเจอ */
                         ...
                         char *buf_out, size_t buf_len)
{
    ...
    if (*p == '"') {
        ...
        size_t i = 0;
        while (*p && *p != '"' && i < buf_len - 1) buf_out[i++] = *p++;
        buf_out[i] = '\0';
        return true;
    } else {
        // number
        size_t i = 0;
        while (*p && (*p == '-' || (*p >= '0' && *p <= '9')) && i < buf_len - 1)
            buf_out[i++] = *p++;
        buf_out[i] = '\0';
        return (i > 0);
    }
...
{
    char buf[16];
    if (json_get_str(json, key, buf, sizeof(buf))) return atoi(buf);
    return def;
}

/* ─────────────────────────────────────────────────────────────
   Parse shadow JSON → s_shadow
   NETPIE shadow resp format:
   {"data":{"scheduleEnabled":1,"t_morn_pre":"08:00",...}}
───────────────────────────────────────────────────────────── */

// slot time keys — ตรงกับ HTML dashboard
...
static uint8_t normalize_med_slots_mask(uint8_t slots_mask)
{
    ...
    for (int base = 0; base <= 4; base += 2) {
        uint8_t pair_mask = (uint8_t)((1 << base) | (1 << (base + 1)));
        if ((slots_mask & pair_mask) == pair_mask) {
            slots_mask &= (uint8_t)~(1 << (base + 1));
        }
    }
...
static void parse_shadow(const char *json)
{
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if (json_get_str(data, s_slot_keys[i], buf, sizeof(buf))) {
            strncpy(s_shadow.slot_time[i], buf, sizeof(s_shadow.slot_time[i]) - 1);
        }
    }
    ...
    for (int i = 0; i < DISPENSER_MED_COUNT; i++) {
        ...
        json_get_str(data, key, s_shadow.med[i].name, sizeof(s_shadow.med[i].name));

        snprintf(key, sizeof(key), "med%d_count", id);
        if (json_get_str(data, key, val_buf, sizeof(val_buf))) {
            int cnt = atoi(val_buf);
            s_shadow.med[i].count = cnt < 0 ? 0 : cnt > DISPENSER_MAX_PILLS ? DISPENSER_MAX_PILLS : cnt;
        }
        ...
        // เราจะไม่ดึง home/work จาก netpie shadow แล้ว เพื่อให้บอร์ดจำค่าจาก Local NVS ของตัวเองแทน (Web API /servo/set)
    }

    // Remote RPC Trigger Listenerสำหรับ "disp_req" (คำสั่งคืนยาผ่านคลาวด์)
    char disp_buf[16];
    if (json_get_str(data, "disp_req", disp_buf, sizeof(disp_buf))) {
        if (strlen(disp_buf) > 0 && strcmp(disp_buf, "0,0") != 0 && strcmp(disp_buf, "0") != 0) {
            int mod_id = 0, qty = 0;
            if (sscanf(disp_buf, "%d,%d", &mod_id, &qty) == 2) {
                if (mod_id >= 1 && mod_id <= DISPENSER_MED_COUNT && qty > 0) {
                    ESP_LOGI(TAG, "NETPIE Triggered Remote Dispense! Mod: %d, Qty: %d", mod_id, qty);
                    extern void dispenser_manual_dispense(int med_idx, int qty);
                    dispenser_manual_dispense(mod_id - 1, qty);
                    
                    // Clear the remote flag to prevent duplicate spawns
                    if (s_client) {
                        char clear_payload[64];
                        snprintf(clear_payload, sizeof(clear_payload), "{\"data\":{\"disp_req\":\"0,0\"}}");
                        esp_mqtt_client_publish(s_client, NETPIE_TOPIC_SET, clear_payload, strlen(clear_payload), 0, 0);
                    }
                }
            }
        }
    ...
    ESP_LOGI(TAG, "Shadow parsed OK — enabled=%d", s_shadow.enabled);
    for (int i = 0; i < DISPENSER_MED_COUNT; i++) {
        if (s_shadow.med[i].name[0]) {
            ESP_LOGI(TAG, "  med%d: \"%s\" count=%d slots=0x%02X",
                     i+1, s_shadow.med[i].name,
                     s_shadow.med[i].count, s_shadow.med[i].slots);
        }
    }
}
                               ...
                               void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        // Subscribe shadow response & updates
    ...
    case MQTT_EVENT_DATA:
        ...
        if (ev->topic_len > 0 && ev->data_len > 0) {
            ...
            if (strstr(topic, "@shadow/data/get/response") || strstr(topic, "@shadow/data/updated")) {
                // copy data เพื่อ null-terminate
...
{
    if (!ensure_shadow_mutex()) {
        ESP_LOGE(TAG, "Failed to create shadow mutex");
        return;
    }

    // default slot times ถ้า shadow ยังไม่มา
    if (!shadow_lock()) return;
    if (!shadow_cache_load_locked()) {
        ...
        for (int i = 0; i < 7; i++) {
            strncpy(s_shadow.slot_time[i], defaults[i],
                    sizeof(s_shadow.slot_time[i]) - 1);
        }
        s_shadow.loaded = true;
...
{
    if (!s_client) return;
    ESP_LOGI(TAG, "Requesting shadow...");
...
void netpie_shadow_update_med_name(int med_id, const char *name)
{
    ...
    char safe_name[sizeof(s_shadow.med[med_id - 1].name)];
    strncpy(safe_name, name, sizeof(safe_name) - 1);
    safe_name[sizeof(safe_name) - 1] = '\0';

    if (!shadow_lock()) return;
    strncpy(s_shadow.med[med_id - 1].name, safe_name, sizeof(s_shadow.med[med_id - 1].name) - 1);
    s_shadow.med[med_id - 1].name[sizeof(s_shadow.med[med_id - 1].name) - 1] = '\0';
    shadow_cache_save_locked();
    shadow_unlock();
...
void netpie_shadow_update_slot(int slot_idx, const char *hh_mm)
{
    ...
    char safe_time[sizeof(s_shadow.slot_time[slot_idx])];
    strncpy(safe_time, hh_mm, sizeof(safe_time) - 1);
    safe_time[sizeof(safe_time) - 1] = '\0';

    if (!shadow_lock()) return;
    strncpy(s_shadow.slot_time[slot_idx], safe_time, sizeof(s_shadow.slot_time[slot_idx]) - 1);
    s_shadow.slot_time[slot_idx][sizeof(s_shadow.slot_time[slot_idx]) - 1] = '\0';
    shadow_cache_save_locked();
    shadow_unlock();
...
bool netpie_publish_shadow_json(const char *payload)
{
    ...
    int msg_id = esp_mqtt_client_publish(s_client, NETPIE_TOPIC_SET, payload, strlen(payload), 0, 0);
