 ...
 *   "<s0>,<s1>,<s2>,<s3>,<s4>,<s5>\n"
 *
 * where each <sN> is the latest distance reading in millimetres (an
 * integer). The value -1 (or any value outside 0..9999) is treated as
 * "no reading / out of range" and surfaced as such on /vl53.
 *
 ...
 * VL53 was abandoned on the P4 bus on 2026-05-14. */
 ...
 * the last successful parse — 0 if no line has been received yet. The
 * caller can compare that tick against xTaskGetTickCount() * portTICK_
 * PERIOD_MS to detect a stale stream. */
 ...
 * (matches the public read-only nature of the sensor data). */
esp_err_t bridge_vl53_html_handler(httpd_req_t *req);

/* GET /vl53.json — JSON snapshot, e.g.
 *   {"ts_ms":12345,"age_ms":250,"mm":[123,456,789,200,300,400]}
 * Used by the HTML page's fetch() poll loop. */
