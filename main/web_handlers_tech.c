...
"<h3 style='margin-top:30px'>โหมดคาริเบรตจริง (Step-by-step)</h3>"
"<p class='sub'>เติมยาเต็มตลับ → กด \"เริ่ม\" → กด \"จ่ายเม็ดถัดไป\" ทีละครั้งจนยาหมด → กด \"คำนวณ &amp; บันทึก\" ระบบจะใช้ระยะจริงทุกขั้นมาคำนวณความสูงเม็ดยาที่ถูกต้องสำหรับช่องนั้น</p>"
...
"</select></label>"
"<label style='font-size:12px;color:#a7bdd7'>จำนวนเริ่มต้น (เม็ด)"
...
"<button class='btn primary' id='cg-start' style='min-height:44px'>▶ เริ่ม</button>"
"<button class='btn ghost' id='cg-step' style='min-height:44px'>💊 จ่ายเม็ดถัดไป</button>"
"<button class='btn primary' id='cg-finish' style='min-height:44px;background:linear-gradient(135deg,#6db8ff,#4dd7b0)'>✓ คำนวณ &amp; บันทึก</button>"
...
"  if(j.active){st.textContent=(extra?extra+' · ':'')+'session: ช่อง '+j.ch+' · เริ่มจาก '+j.max_pills+' เม็ด · บันทึกแล้ว '+j.steps.length+'/'+(j.max_pills+1)+' จุด';st.style.color='#9ae8d0';}"
"  else if(j.steps&&j.steps.length){st.textContent=(extra?extra+' · ':'')+'เสร็จสิ้น '+j.steps.length+' จุด';st.style.color='#9ae8d0';}"
...
"async function cgRefresh(){const ch=document.getElementById('cg-ch').value;"
"  try{const r=await fetch('/sensors/cal_guide?ch='+ch);const j=await r.json();cgStateText(j,'');cgRenderTable(j);}catch(e){}}"
"document.getElementById('cg-ch').addEventListener('change',cgRefresh);"
"document.getElementById('cg-start').addEventListener('click',async()=>{"
"  const ch=document.getElementById('cg-ch').value;const mx=document.getElementById('cg-max').value;"
"  if(!confirm('เริ่มคาริเบรตช่อง '+ch+' ที่ '+mx+' เม็ด? (ต้องเติมยาเต็มก่อน)'))return;"
"  cgStateText(null,'กำลังเริ่ม…');"
"  try{const r=await fetch('/sensors/cal_guide?ch='+ch+'&action=start&max_pills='+mx,{method:'POST'});const j=await r.json();cgStateText(j,'เริ่มแล้ว ✓');cgRenderTable(j);}catch(e){cgStateText(null,'ขัดข้อง');}});"
"document.getElementById('cg-step').addEventListener('click',async()=>{"
"  const ch=document.getElementById('cg-ch').value;cgStateText(null,'กำลังจ่าย 1 เม็ด + วัดระยะ ใช้เวลา ~10 วิ');"
"  try{const r=await fetch('/sensors/cal_guide?ch='+ch+'&action=step',{method:'POST'});const j=await r.json();cgStateText(j,'จ่ายเสร็จ ✓');cgRenderTable(j);}catch(e){cgStateText(null,'ขัดข้อง');}});"
"document.getElementById('cg-finish').addEventListener('click',async()=>{"
"  const ch=document.getElementById('cg-ch').value;"
"  if(!confirm('คำนวณ + บันทึก? (จะเขียนทับค่า full_dist + height + max_pills เดิม)'))return;"
"  try{const r=await fetch('/sensors/cal_guide?ch='+ch+'&action=finish',{method:'POST'});const j=await r.json();"
"    if(j.ok){const st=document.getElementById('cg-state');st.style.color='#9ae8d0';st.textContent='บันทึกแล้ว ✓ '+j.steps+' จุด → full='+j.full_dist_mm+'mm height='+j.pill_height_mm+'mm max='+j.max_pills;document.getElementById('cg-table').innerHTML='';calRender();}"
"    else{cgStateText(j,'');}}"
"  catch(e){cgStateText(null,'ขัดข้อง');}});"
"document.getElementById('cg-reset').addEventListener('click',async()=>{"
"  const ch=document.getElementById('cg-ch').value;"
"  try{const r=await fetch('/sensors/cal_guide?ch='+ch+'&action=reset',{method:'POST'});const j=await r.json();cgStateText(j,'ยกเลิก');document.getElementById('cg-table').innerHTML='';}catch(e){}});"
"cgRefresh();"
