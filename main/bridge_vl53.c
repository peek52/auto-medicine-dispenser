 ...
 * Protected by s_lock so the web layer reading and the task writing
 * never see a half-updated array. */
static int      s_dist_mm[BRIDGE_VL53_NUM_SENSORS] = {-1, -1, -1, -1, -1, -1};
static uint32_t s_last_update_ms = 0;
static SemaphoreHandle_t s_lock  = NULL;
static bool     s_started        = false;

/* Receive line buffer — one CSV line "123,456,789,200,300,400\n" fits
 * comfortably under 64 chars even with negative/four-digit values. */
#define LINE_BUF_SIZE 96

/* Tear apart a "<int>,<int>,...,<int>" line into BRIDGE_VL53_NUM_SENSORS
 * integers. Returns true only if we got the expected count AND every
 * field parsed. Permissive about leading/trailing whitespace and an
 * optional "vl53:" prefix so the C3 firmware can include it for
 * grep-ability on a serial monitor. */
static bool parse_csv_line(const char *line, int out[BRIDGE_VL53_NUM_SENSORS])
{
    if (!line) return false;
    /* Skip optional "vl53:" prefix. */
...
static void bridge_vl53_task(void *arg)
{
    ...
    /* RX buffer 1024 bytes is plenty — the CSV line is < 64 chars and we
     * drain it every tick. No TX buffer; we don't transmit anything. */
    ...
    while (1) {
        ...
        if (n <= 0) {
            /* No data yet — keep waiting. Task is otherwise idle so the
             * 200 ms poll keeps CPU pressure negligible. */
            continue;
        }

        if (byte == '\r') continue;   /* Ignore CR; accept LF as terminator. */
        if (byte == '\n') {
            line[fill] = '\0';
            ...
            if (parse_csv_line(line, parsed)) {
                ...
                /* Don't log every malformed frame — too chatty if the C3
                 * is still booting and emits boot banners. Only log
                 * lines that look like they were trying to be data. */
                if (strchr(line, ',') != NULL) {
        ...
        if (fill < sizeof(line) - 1) {
            ...
            /* Overflow — reset and drop the rest of this line. */
            fill = 0;
        }
    }
}
...
/* ── Web layer ──────────────────────────────────────────────────────
 * Self-contained — does NOT touch the dispense/IR/schedule code or any
 * auth cookie state. Public read-only because the sensor data carries
 * no PII and the page is meant for demos. */
...
"<div class=\"sub\">ค่าระยะที่ได้รับจาก ESP32-C3 (TCA9548A + 6\xc3\x97 VL53L0X) ผ่าน UART2</div>"
"<div class=\"grid\" id=\"grid\"></div>"
"<div class=\"meta\" id=\"meta\"><span class=\"dot dead\"></span>กำลังรอข้อมูล...</div>"
"</div><script>"
...
"const ageStr=d.ts_ms===0?'ยังไม่ได้รับเฟรมจาก C3':(d.age_ms<1000?'อัปเดตเมื่อ <1 วินาทีที่แล้ว':"
"'อัปเดตเมื่อ '+Math.round(d.age_ms/1000)+' วินาทีที่แล้ว');"
"M.innerHTML='<span class=\"'+dotCls+'\"></span>'+ageStr;}"
"catch(e){M.innerHTML='<span class=\"dot dead\"></span>เชื่อมต่อเครื่องไม่ได้';}}"
