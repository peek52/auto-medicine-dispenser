...
esp_err_t web_redirect_to_cloud_login(httpd_req_t *req, bool clear_session)
{
    ...
    snprintf(redirect, sizeof(redirect), "http://%s/cloud", host);
...
esp_err_t web_require_tech_page_auth(httpd_req_t *req)
{
    ...
    return maintenance_send_login_page(req, false);
}
...
esp_err_t cloud_login_handler(httpd_req_t *req)
{
    ...
    if (!cloud_secrets_verify_cloud_access_code(code) &&
    ...
    char cookie[128];
    snprintf(cookie, sizeof(cookie), "cloud_auth=%s; Path=/; HttpOnly; SameSite=Lax", s_cloud_session_token);
    httpd_resp_set_status(req, "303 See Other");
...
esp_err_t cloud_setup_handler(httpd_req_t *req) {
    if (!cloud_auth_cookie_valid(req)) {
        return cloud_send_login_page(req, false);
    }
    ...
    snprintf(html, 12288,
        ...
        "</div>"
        "<div class='actions'>"
        "<button class='btn btn-primary' type='submit'>Save Cloud Settings</button>"
        "<a class='btn btn-secondary' href='/cloud/test?mode=message'>Test Telegram Message</a>"
        "<a class='btn btn-secondary' href='/cloud/test?mode=photo'>Test Telegram Photo</a>"
        "</div>"
        "</form>"
        "<div class='note'>Save first, then run a Telegram test. This page now keeps only cloud-related settings.</div>"
        "</section>"
        "</div>"
        "</body></html>",
